...
function displayPics(){
  ...
  //Let is a locally scoped lexical argument and does not create a global property (thus it only works between the while bracket it's defined in). Since console.log is asking for rando from locally scoped variable to displayPics it throws and error.
  console.log(viewed)
...
function handleClick(event) {
  ...
  for(let i = 0; i < names.length; i++){
    if(event.target.id === allProducts[i].name) {
      ...
      console.log(`${event.target.id} has ${allProducts[i].votes} votes in ${allProducts[i].view} views`);
    }
  }
