...
function displayPics(){
  ...
  // The reason console.log(rando); didn't work anymore is because var has functional/contextual scope whereas let has strictly block scope. With var, JS will basically pretend there is a rando declared in the same scope as the console log, and thus we'll be able to log its value at the current time. With LET however, there must be an additional let rando = x; declared in the same scope as the console log in order for it to return a value. If the console log was inside the while loop is was declared in, it would be possible to get its value but since the console.log(rando); is in the function but not inside the loop where let rando is declared, it will return undefined.
  console.log(viewed)
